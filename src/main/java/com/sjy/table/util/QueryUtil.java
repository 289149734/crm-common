...
public final class QueryUtil {
	...
	public static String getHint(SqlQuery query, String addHint) {
		...
		if (hint == null && addHint == null) return "";

		if (hint == null) hint = "";
		if (addHint != null) hint = addHint + " " + hint;
		return "/*+" + hint + "*/";
	}
	...
	public static String getTitle(SqlQuery query, String locale) {
		...
		try {
			...
			if (b != null) b.setProperty("_c_locale", locale == null ? "zh" : locale);
			title = query.dynaTitle.run().toString();
		} catch (Exception e) {
			throw new CrmException("解析动态title出错:" + query.title, e);
		}
	...
	public static QueryTranslatorImpl compileSql(SessionFactory sessionFactory, String originalHql) {
	...
	public static List<Object[]> parseSqlResult(SessionFactory sessionFactory, ResultSet rs, int colSize, int maxResult,
			QueryTranslatorImpl translator) throws SQLException {
		List<Object[]> l = maxResult <= 200 ? new ArrayList<Object[]>(maxResult) : new ArrayList<Object[]>();
		Object[] os;
		if (translator == null) {
			...
			for (int k = 0; rs.next() && k < maxResult; k++) {
				...
				for (int k1 = 0; k1 < colSize; k1++) {
					type = types[k1];
					if (type != null) os[k1] = type.nullSafeGet(rs, alias[k1], (SessionImplementor) sessionFactory.getCurrentSession(), null);
					else os[k1] = rs.getObject(k1 + 1);
				}
				l.add(os);
			}
		}
